          ...
          return (
            <div key={seller.id} className="seller-card">
              ...
              <div className="seller-header">
                {/* ВИПРАВЛЕНО: Використовуємо seller.slug замість product.sellerSlug */}
                <Link to={`/markets/${seller.slug || seller.id}`}>
                  <img src={seller.logoUrl} alt={seller.name} className="seller-logo" />
                </Link>
                
                <div className="seller-info">
                  {/* ВИПРАВЛЕНО: Використовуємо seller.slug */}
                  <Link to={`/markets/${seller.slug || seller.id}`} style={{ textDecoration: 'none' }}>
                  ...
                  <div className="seller-contacts">
                    <a href={`tel:${seller.phone}`} className="contact-item">
                      <svg className="contact-icon" style={{ width: '18px', height: '18px', fill: 'currentColor' }}>
              ...
              <svg xmlns="http://www.w3.org/2000/svg" style={{ display: 'none' }}>
                <symbol id="arrow-icon" viewBox="0 0 14 12">
                  <path d="M8.5 0.5L7.09 1.91L10.67 5.5H0V7.5H10.67L7.09 11.09L8.5 12.5L14.5 6.5L8.5 0.5Z"/>
                </symbol>
              </svg>
              <div className="seller-products-section">
                <div className="seller-products-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                  <h4 style={{ margin: 0 }}>Товари магазину</h4>
                  {/* ВИПРАВЛЕНО: Використовуємо seller.slug */}
                  <Link to={`/markets/${seller.slug || seller.id}`} className="view-all-link" style={{ color: '#D97757', textDecoration: 'none', fontWeight: 'bold' }}>
                    Всі товари майстра
                ...
                {sellerProducts.length > 0 ? (
                  <div className="products-grid" style={{ paddingLeft: '0px', paddingRight: '0px' }}>
                    {/* ВИПРАВЛЕНО: Змінив змінну на product, щоб не було помилки "product is not defined" у <ProductCard> */}
                  ...
                  <div className="products-placeholder">
                    <p>Тут незабаром з'являться картки товарів від "{seller.name}"...</p>
                  </div>
                )}
              </div>
